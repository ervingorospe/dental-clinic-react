...
const ServiceSelectInput: React.FC<ServiceSelectInputProps> = ({ selectedService, setSelectedService, services }) => {
  return (
    <Listbox value={selectedService} onChange={setSelectedService}>
      <Label className="block text-sm/6 font-medium text-gray-900">Select a Service</Label>
      <div className="relative mt-2">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-2 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
          {
            selectedService && (
              <span className="col-start-1 row-start-1 flex w-full gap-2 pr-6">
                <span className="truncate">{ selectedService.category.name ?? '' } - {selectedService.name ?? ''}</span>
              </span>
            )
          }
          <ChevronUpDownIcon
        ...
        >
          {services.map((service: any) => (
            ...
            >
              <div className="flex">
                <span className="truncate font-normal group-data-selected:font-semibold">{service.category.name} - {service.name} </span>
              </div>
